...
export type FunctionInfo = FunctionData;
export type CallInfo = CallData;
export type CallSite = CallSiteNode;
  ...
  lines.forEach((line, index) => {
    ...
    functionNames.forEach((fnName) => {
      ...
      if (callRegex.test(line)) {
        ...
        // Determine if this call is inside a function by checking indentation
        // Python uses indentation to define scope
        const lineIndent = line.search(/\S/); // Find first non-whitespace character

        // Check if we're inside any function's body by comparing indentation
        for (let i = functions.length - 1; i >= 0; i--) {
          const func = functions[i];

          // Only consider functions defined before this line
          if (func.lineNumber >= lineNumber) {
            continue;
          }

          // Get the indentation of the function definition line
          const funcLine = lines[func.lineNumber - 1];
          const funcIndent = funcLine.search(/\S/);

          // If the call line is indented more than the function definition,
          // it's inside that function's body
          if (lineIndent > funcIndent && func.name !== fnName) {
            caller = func.name;
            break;
          }
        }

...
interface FunctionVisualizerProps {
  ...
  onExport?: (type: "json" | "jpeg", content: any) => void;
}
...
export function FunctionVisualizer({
  ...
  const handleExportJSON = () => {
    ...
    // If running in VS Code webview, use the callback
    if (onExport) {
      onExport("json", data);
    } else {
  ...
  const handleExportImage = async () => {
    if (callGraphRef.current && viewMode === "graph") {
      if (onExport) {
        // For VS Code, export using the callback
        const imageData = await callGraphRef.current.getImageData();
        onExport("jpeg", imageData);
      } else {
        // For browser, use the direct export
